...
struct ListObjectsArgs {
    ...
    /// `json` emits `[{"name","slug"}]` for a CI matrix; `lines` emits
    /// `<slug>\t<name>` for shell consumption.
...
/// Resolve `--populations` / `--only` to a concrete object set.
///
/// Shared by `plan` and `list-objects` on purpose: the CI matrix is generated
/// from `list-objects` and every leg's reference shard is generated by `plan`
/// restricted to one of those names. If the two resolved their filters
/// independently they could drift, and the failure would be an object that the
/// matrix never covers — silently absent from the report rather than loudly
/// missing. One function, one answer.
fn select_objects(
    populations: &[String],
    only: &[String],
) -> Result<Vec<&'static ValidationObject>, Box<dyn std::error::Error>> {
    let pool: Vec<&ValidationObject> = if populations.is_empty() {
        all_objects()
    } else {
        let pops: Vec<&str> = populations.iter().map(String::as_str).collect();
        filter_by_population(&pops)
    };
    let selected: Vec<&ValidationObject> = if only.is_empty() {
        pool
    } else {
        let names: Vec<&str> = only.iter().map(String::as_str).collect();
        let by_name = filter_by_name(&names);
        // An `--only` name that matches nothing in the catalog is a typo or a
        // renamed object, not a request for fewer objects. Say so, naming it.
        let unmatched: Vec<&str> = names
            ...
            .filter(|n| {
                !by_name
                    .iter()
                    .any(|o| o.name.eq_ignore_ascii_case(n.trim()))
            })
            .collect();
        if !unmatched.is_empty() {
            return Err(format!(
                "--only named {} object(s) that are not in the catalog: {}. \
                 A misspelled object would otherwise silently narrow the run.",
                unmatched.len(),
                unmatched.join(", ")
            )
            .into());
...
/// Filesystem- and CI-safe token for an object name.
///
/// Object names carry spaces, slashes and apostrophes (`46P/Wirtanen`,
/// `1I/'Oumuamua`), none of which survive a shell word, an artifact name or a
/// path component intact. Anything outside `[A-Za-z0-9._-]` collapses to `_`.
...
fn list_objects(args: ListObjectsArgs) -> Result<(), Box<dyn std::error::Error>> {
    let selected = select_objects(&args.populations, &args.only)?;

    // A slug collision would make two objects share one shard directory and
    // one artifact name: the second silently overwrites the first, and the
    // report shows a full catalog while one object was never actually run.
    // Refuse to emit a matrix that could do that.
    let mut seen: std::collections::HashMap<String, &str> = std::collections::HashMap::new();
    for o in &selected {
        let slug = object_slug(o.name);
        if let Some(prev) = seen.insert(slug.clone(), o.name) {
            return Err(format!(
                "object slug collision: {:?} and {:?} both slugify to {:?}. \
                 They would share a shard directory and one would overwrite \
                 the other. Rename one, or widen object_slug().",
                prev, o.name, slug
            )
            .into());
        }
    ...
    match args.format.as_str() {
        "json" => {
            let rows: Vec<serde_json::Value> = selected
                .iter()
                .map(|o| serde_json::json!({ "name": o.name, "slug": object_slug(o.name) }))
                .collect();
            println!("{}", serde_json::to_string(&rows)?);
        }
        "lines" => {
            for o in &selected {
                println!("{}\t{}", object_slug(o.name), o.name);
            }
        }
        other => {
            return Err(format!("unknown --format {other:?} (expected `json` or `lines`)").into());
        }
    }
    eprintln!("Listed {} objects", selected.len());
    Ok(())
}
...
mod tests {
    ...
    fn merge_grss_carries_the_model_note_on_every_axis() {
        ...
        for (i, r) in rows.iter().enumerate() {
            assert!(
                r.grss_model_note.as_deref().is_some_and(|s| s.contains("no DT term")),
    ...
    fn merge_grss_carries_the_failure_reason_not_a_blank() {
        ...
        let n = merge_grss(&mut rows, f.path()).unwrap();
        assert_eq!(n, 1);
        assert!(rows[0].grss_error.as_deref().unwrap().contains("did not converge"));
        assert_eq!(rows[0].grss_rms_arcsec, None);
    ...
    // ── Per-object CI fan-out: slugs and selection ──────────────────

    #[test]
    fn slug_survives_the_characters_real_object_names_carry() {
        // Spaces, slashes and apostrophes all appear in the live catalog and
        // none of them survives a shell word, an artifact name or a path
        // component intact.
        assert_eq!(object_slug("2024 YR4"), "2024_YR4");
        assert_eq!(object_slug("46P/Wirtanen"), "46P_Wirtanen");
        assert_eq!(object_slug("1I/'Oumuamua"), "1I__Oumuamua");
        assert_eq!(object_slug("103P/Hartley 2"), "103P_Hartley_2");
        // Already-safe characters pass through untouched.
        assert_eq!(object_slug("Apophis"), "Apophis");
        assert_eq!(object_slug("a.b-c_d"), "a.b-c_d");
    }

    #[test]
    fn every_catalog_slug_is_unique() {
        // This is the invariant the CI fan-out rests on: each object gets one
        // matrix leg, one shard directory and one artifact name, all keyed by
        // slug. A collision would make two objects share a shard directory so
        // the second silently overwrites the first — and the report would show
        // a full catalog while one object was never actually run.
        let mut seen: std::collections::HashMap<String, &str> = std::collections::HashMap::new();
        for o in all_objects() {
            let slug = object_slug(o.name);
            assert!(
                !slug.is_empty(),
                "object {:?} slugified to the empty string",
                o.name
            );
            if let Some(prev) = seen.insert(slug.clone(), o.name) {
                panic!(
                    "slug collision: {:?} and {:?} both give {slug:?}",
                    prev, o.name
                );
            }
        }
        assert_eq!(seen.len(), all_objects().len());
    }

    #[test]
    fn unmatched_only_name_is_an_error_not_a_narrower_run() {
        // A misspelled object in --only used to silently shrink the run: the
        // matrix would simply not cover it, and nothing downstream could tell
        // "not requested" from "requested and lost".
        let err = select_objects(&[], &["Apophis".into(), "Aphophis".into()])
            .expect_err("a name absent from the catalog must be rejected");
        let msg = err.to_string();
        assert!(
            msg.contains("Aphophis"),
            "error must name the offender: {msg}"
        );
    }

    #[test]
    fn selection_is_case_insensitive_and_order_independent() {
        let a = select_objects(&[], &["apophis".into(), "BENNU".into()]).unwrap();
        let b = select_objects(&[], &["Bennu".into(), "Apophis".into()]).unwrap();
        assert_eq!(a.len(), 2);
        let names_a: std::collections::HashSet<&str> = a.iter().map(|o| o.name).collect();
        let names_b: std::collections::HashSet<&str> = b.iter().map(|o| o.name).collect();
        assert_eq!(names_a, names_b);
    }

    #[test]
    fn empty_selection_covers_the_whole_catalog() {
        // The CI default is an empty OBJECTS, and it must mean "everything" —
        // an empty filter that quietly resolved to an empty set would fan out
        // zero legs and produce an empty-but-green run.
        assert_eq!(select_objects(&[], &[]).unwrap().len(), all_objects().len());
    }
}
